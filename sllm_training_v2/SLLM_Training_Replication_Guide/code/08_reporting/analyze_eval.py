...
EVAL_DIR = r"D:\Dev\custom_llm_outputs\mixed"
TRAIN_SOURCE_DIR = r"C:\Users\mandar.ukrulkar\Downloads\data_raw_in_out_jsons_200726\output"
OUT_JSON = r"D:\Dev\sllm_training_v2\report_assets\eval_stats.json"
...
jobs = []
missing_compare = []
per_table_rows = []  # flattened across all jobs
job_docnames = {}
...
# ---- best individual tables: perfect (or near-perfect) score on every measured field,
# ranked by table size so the "best" list highlights genuine complexity handled well,
# not trivial 1-2 row tables that are easy to get right ----
...
# ---- in-sample (job_id seen in training source corpus) vs held-out generalization check ----
in_sample_jobs = [j for j in jobs if j["job_id"] in TRAIN_JOB_IDS]
held_out_jobs = [j for j in jobs if j["job_id"] not in TRAIN_JOB_IDS]


def summarize_bucket(job_list):
    row_vals, tt_lex_vals, col_vals, cid_lex_vals = [], [], [], []
    docs = {j["docname"] for j in job_list}
    n_tables = 0
    for pt in per_table_rows:
        owner = next((j for j in job_list if j["job_id"] == pt["job_id"]), None)
        if owner is None:
            continue
        n_tables += 1
        tt_lex_vals.append(1 if pt.get("table_type_lexical_match") else 0)
        if pt.get("row_type_accuracy") is not None:
            row_vals.append(pt["row_type_accuracy"])
        if pt.get("column_type_accuracy") is not None:
            col_vals.append(pt["column_type_accuracy"])
        if pt.get("concept_id_lexical_accuracy") is not None:
            cid_lex_vals.append(pt["concept_id_lexical_accuracy"])
    return {
        "n_jobs": len(job_list),
        "n_docs": len(docs),
        "n_tables": n_tables,
        "table_type_lexical_match_rate": statistics.mean(tt_lex_vals) if tt_lex_vals else None,
        "row_type_accuracy": statistics.mean(row_vals) if row_vals else None,
        "column_type_accuracy": statistics.mean(col_vals) if col_vals else None,
        "concept_id_lexical_accuracy": statistics.mean(cid_lex_vals) if cid_lex_vals else None,
    }


generalization_check = {
    "in_sample": summarize_bucket(in_sample_jobs),
    "held_out": summarize_bucket(held_out_jobs),
}

# ---- deployment cost analysis (NVIDIA T4, $384/month, billed per second) ----
DEPLOYMENT_MONTHLY_USD = 384.0
HOURS_PER_MONTH = 730.0  # standard cloud-vendor average-month convention
SECONDS_PER_MONTH = HOURS_PER_MONTH * 3600.0
RATE_PER_SECOND = DEPLOYMENT_MONTHLY_USD / SECONDS_PER_MONTH

mean_lat = per_table_latency and statistics.mean(per_table_latency)
median_lat = per_table_latency and statistics.median(per_table_latency)
capacity_tables_per_month = SECONDS_PER_MONTH / mean_lat if mean_lat else None
full_corpus_financial_tables = 6260  # from the training-corpus analysis (analyze.py), all 253 source jobs

cost_analysis = {
    "deployment_monthly_usd": DEPLOYMENT_MONTHLY_USD,
    "hours_per_month_assumed": HOURS_PER_MONTH,
    "rate_per_second_usd": RATE_PER_SECOND,
    "rate_per_hour_usd": RATE_PER_SECOND * 3600,
    "marginal_cost_per_table_mean_usd": mean_lat * RATE_PER_SECOND if mean_lat else None,
    "marginal_cost_per_table_median_usd": median_lat * RATE_PER_SECOND if median_lat else None,
    "cost_of_this_eval_run_usd": (sum(times) * RATE_PER_SECOND) if times else None,
    "capacity_tables_per_month_theoretical": capacity_tables_per_month,
    "full_corpus_financial_tables": full_corpus_financial_tables,
    "full_corpus_processing_hours": (full_corpus_financial_tables * mean_lat / 3600) if mean_lat else None,
    "full_corpus_processing_cost_usd": (full_corpus_financial_tables * mean_lat * RATE_PER_SECOND) if mean_lat else None,
    "typical_doc_tables_mean": 25.04,   # mean financial_table_count per job, full source corpus
    "typical_doc_tables_median": 11.5,  # median financial_table_count per job, full source corpus
    "typical_doc_seconds_mean": (25.04 * mean_lat) if mean_lat else None,
    "typical_doc_seconds_median": (11.5 * mean_lat) if mean_lat else None,
    "large_doc_tables": 222,  # largest single document in the source corpus
    "large_doc_seconds": (222 * mean_lat) if mean_lat else None,
}
...
print("in_sample:", generalization_check["in_sample"])
print("held_out:", generalization_check["held_out"])
print()
print("=== cost_analysis ===")
